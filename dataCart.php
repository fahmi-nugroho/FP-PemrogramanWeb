...
if (isset($_POST['action'])) {
    ...
    elseif ($action == "viewCheckout") {
        ...
        <dl class="row">
            ...
            <div class="col-lg-4">
                <select class="custom-select custom-select-sm" id="ongkir" onchange="ongkir(this.value, <?= count($toko) ?>, <?= $total ?>)">
    ...
    elseif ($action == "checkout") {
        ...
        for ($i = 0; $i < count($toko); $i ++) {
            ...
            if ($bayar == "Wallet" && $data['wallet'] >= $totalpt) {
                $wallet = $data['wallet'] - $totalpt;
        ...
        unset($_SESSION['cart']);
        if ($bayar == "Wallet") {
            echo "Saldo Wallet Telah Dikurangi Sesuai Total Pembelian";
        } else {
